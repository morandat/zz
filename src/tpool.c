...
/*
 * Modified to work with z2z gateway
 */ 
        ...
        int max_queue_size, int do_not_block_when_full)
{
    ...
    {
      ERR("Unable to malloc() thread pool!: %s\n",strerror(errno));
        return NULL;
    }
    ...
    {
      ERR("Unable to malloc() thread info array: %s\n",strerror(errno));
        return NULL;
    }
    ...
    if((rtn = pthread_mutex_init(&(pool->queue_lock),NULL)) != 0) {
      ERR("pthread_mutex_init %s",strerror(rtn));
        return NULL;
    }
    if((rtn = pthread_cond_init(&(pool->queue_not_empty),NULL)) != 0) {
      ERR("pthread_cond_init %s",strerror(rtn));
        return NULL;
    }
...
int tpool_add_work(tpool_t *pool, void (*routine)(), void *arg)
{
    ...
    {
      ERR("pthread mutex lock failure : %s\n",strerror(rtn));
        return -1;
    }
            ...
            (pool->do_not_block_when_full))
    {
        ...
        {
	  ERR("pthread mutex lock failure\n");
            return -1;
        }
        return -1;
    }
    ...
    if((workp = (tpool_work_t *)malloc(sizeof(tpool_work_t)))
            == NULL)
    {
      ERR("unable to create work struct\n");
        return -1;
    }
...
//    printf("Add work> number of messages to send in the queue:%i",pool->cur_queue_size);
    return 0;
}

int tpool_wait_working_thread(tpool_t *pool)
{
  int rtn,i;

  printf("Wait for the working threads");
  /* relinquish control of the queue */
  if((rtn = pthread_mutex_lock(&(pool->queue_lock))) != 0)
    {
      ERR("pthread mutex lock failure\n");
  ...
  for(i = 0; i < pool->num_threads; i++)
    {
      printf("Wait working thread> wait for threads:%i",i);
      //printf("join %i", i);
  ...
  if((rtn = pthread_mutex_unlock(&(pool->queue_lock))) != 0)
    {
      ERR("pthread mutex unlock failure\n");
      return -1;
    }
  printf("No waiting threads anymore");
}
